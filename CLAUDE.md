...
| 15 | $5K options capital plan | written 2026-05-05. Two paths: (1) deploy doc-14's validated SPY+PCS+IC+JL+IV<0.25 at $5K capital — reproduced 16yr walk-forward 2026-05-05 at OOS Sharpe +0.867 / +$8.8K / 13.5% AnnRoC, gate PASSES, deployable today on existing data; (2) MES futures options 0DTE (intraday-compliant, NOT yet validated). **Data path resolved 2026-05-05: existing databento `GLBX.MDP3` covers MES options. NO Polygon/CBOE needed.** Two parent symbology trees verified: `MES.OPT` quarterlies (~$0.59/yr; useless for 0DTE — only 4 expiries/yr), and `X[1-5][A-D].OPT` daily expirations (A=Mon..D=Thu, 20 parents total, ~$2.64/yr for full grid `ohlcv-1m`+`definition`). Full retail coverage: 2024 ≈ $3.23, 5yr (2020-2024, dailies launched Sep 2022) ≈ $8.88. **Three data slices pulled 2026-05-05/06**: 2024 quarterlies, 2023-2024 X-prefix dailies, AND Jan-Apr 2025 holdout (all 21 parents × 2 schemas, $1.59 spent — 1.05M total rows in mes_options_chain source covering 2023-02-13 → 2025-04-30). **Total cumulative spend $6.78.** **0DTE infrastructure shipped + pre-registered OOS PASSED**: ingest module + chain reader + 0DTE harness (with intraday PT/LS) + Mes0dteIronCondor + Mes0dtePcs + VixGatedStrategy wrapper. **Live IBKR daemon shipped 2026-05-06** (`scripts/live_mes_0dte.py` + launchd plists + auto-close + force-close + kill-switch).  Initially gated spec was IC $25/$25 + VIX>18 + PT 50% (in-sample +$708, OOS +$1,427).  **Daemon switched 2026-05-06 to DAILY-FIRE: IC $10/$10 + PT 75% + no gate, entry 10:00 ET** — same per-trade EV but 4× more trades (162/yr); IS +$2,731 net / 75% WR / +$8.43/T, OOS +$954 net / 74.6% WR / +$16.17/T, all 6 sub-windows positive including April 2025 stress month.  Live IBKR probe verified 2026-05-06: connect → qualify front-month + 4 daily-options legs (X1C/X1D trading classes) → snapshot bid/ask works.  VIX index queried via delayed feed (paper account doesn't have live CBOE sub).  See `trading_platform_docs/15_5k_options_capital_plan.md`. |
| 14 | Options vol selling | Phases A+B+C+D-1..D-7 complete 2026-05-03; **Phase D-8 (walk-forward + CPCV harness) shipped 2026-05-03 with discriminating findings**. **Bare strategies FAIL the discipline gate** (PCS solo, PCS+IC portfolio, JL+tight all have negative IS Sharpe in every rolling 3y window — each window includes 2022). **IV-gated wrappers SURVIVE**: `PCS + IV<0.30` (counter-canonical — sell vol when IV is LOW, NOT high) PASSES walk-forward at avg IS +0.022 / OOS +0.163; `PCS + IC + IV<0.30` portfolio is the cleanest validated config with avg IS +0.141 / OOS +0.137 (ratio 0.97), all 6 sub-windows positive, ~11.2% AnnRoC during OOS years on $250K. CPCV (45 paths): portfolio + IV<0.30 has 91.1% paths positive (median Sharpe +0.249, vs ungated portfolio +0.036). The canonical practitioner rule "PCS + IV>0.50" FAILS spectacularly. 629 tests passing. Live IBKR contract qualification verified on paper account DU7535411. **Threshold robustness sweep**: gate passes monotonically across IV<0.20, <0.25, <0.30, <0.35; fails at <0.40 and at the canonical >0.50. NOT a knife-edge. **Composition sweep at IV<0.25**: 3-strategy PCS+IC+JL portfolio is best — avg OOS Sharpe +0.275, worst OOS Sharpe +0.135 (all 3 OOS windows positive), +$130K OOS over 3y (~17.3% AnnRoC on $250K). **$25K WINNER**: SPY+PCS+IC+JL+IV<0.25 → ~28% AnnRoC OOS (avg IS +0.172, OOS +0.254, all 6 sub-windows positive, 6yr 2020-2026 walk-forward). 16-yr SPY walk-forward (2010-2025, 13 windows) revealed worst-window OOS -1.407 — tail risk that 6yr period hid. **$5K candidate (2026-05-04, regime-fragile)**: **EEM solo + PCS+IC+JL + IV<0.25** passes 6-yr (2020-2026) walk-forward at avg IS +0.193 / OOS +0.250 / worst OOS +0.027 / 160 OOS trades/yr / +$6,737 OOS = ~45% AnnRoC. **BUT 16-yr walk-forward (2010-2026) FAILS** — avg IS +0.013 / OOS -0.003 / worst OOS -0.596. Pre-2020 OOS sums to **-$873** vs post-2020 +$7,855. The 2020+ regime (COVID + supply chain + geopolitics → elevated EM IV) is what makes the strategy work; 2010-2019 EM vol spikes (Eurozone 2011, China 2015, Brexit 2016, trade war 2018) crushed short-vol. SPY+IWM+QQQ+EEM combo also passes 6-yr (+$10.5K OOS) but year variance is huge (2024 windfall). 7-underlying all-in (SPY/IWM/QQQ/DIA/GLD/TLT/XLE/EEM) FAILS gate — diversification dilution kills it. **Recommended deploy after full 16-yr diligence (2026-05-04 update)**: **$25K → SPY+PCS+IC+JL+IV<0.25 (16yr OOS Sharpe +0.138, defensible)**. **$5K → SPY same config, NOT EEM**. Why SPY at $5K beats EEM at $5K: SPY 16yr walk-forward OOS Sharpe +0.867 (noisy small-count windows but consistently positive across 13 OOS years, total +$8.8K = ~13% AnnRoC), gate PASSES. EEM 16yr walk-forward FAILS — pre-2020 OOS is -$873; the 6-yr ~45% headline was a 2020+ regime artifact. SPY at $5K is capital-constrained (19.8 OOS trades/yr vs $25K's 68) but the trades that fire are profitable across all regimes. Both $5K and $25K use same SPY+PCS+IC+JL+IV<0.25 strategies + 50/21/200 mgmt + IBKR paper account DU7535411 — just different `--capital` flag. |
