...
| 14 | Options vol selling | Phases A+B+C+D-1..D-7 complete 2026-05-03; **Phase D-8 (walk-forward + CPCV harness) shipped 2026-05-03 with discriminating findings**. **Bare strategies FAIL the discipline gate** (PCS solo, PCS+IC portfolio, JL+tight all have negative IS Sharpe in every rolling 3y window — each window includes 2022). **IV-gated wrappers SURVIVE**: `PCS + IV<0.30` (counter-canonical — sell vol when IV is LOW, NOT high) PASSES walk-forward at avg IS +0.022 / OOS +0.163; `PCS + IC + IV<0.30` portfolio is the cleanest validated config with avg IS +0.141 / OOS +0.137 (ratio 0.97), all 6 sub-windows positive, ~11.2% AnnRoC during OOS years on $250K. CPCV (45 paths): portfolio + IV<0.30 has 91.1% paths positive (median Sharpe +0.249, vs ungated portfolio +0.036). The canonical practitioner rule "PCS + IV>0.50" FAILS spectacularly. 629 tests passing. Live IBKR contract qualification verified on paper account DU7535411. **Threshold robustness sweep**: gate passes monotonically across IV<0.20, <0.25, <0.30, <0.35; fails at <0.40 and at the canonical >0.50. NOT a knife-edge. **Composition sweep at IV<0.25**: 3-strategy PCS+IC+JL portfolio is best — avg OOS Sharpe +0.275, worst OOS Sharpe +0.135 (all 3 OOS windows positive), +$130K OOS over 3y (~17.3% AnnRoC on $250K). **$25K-DEPLOYABLE WINNER FOUND — SPY ETF options**: SPY 6-year ORATS chain pulled + ingested 2026-05-03 (3.5 GB, 6.9M rows, 1508 trade days). At $25K with 3-strategy `PCS + IC + JL + IV<0.25` portfolio: **~28.4% AnnRoC OOS** over 3 years (+$21.3K OOS PnL), avg IS Sharpe +0.172 / OOS +0.254 (ratio 1.48), all 6 sub-windows positive, ~68 OOS trades/year. **Beats even SPX@$250K's 17%** in % terms because SPY's economics work at retail size: $100 multiplier + ~1/10-SPX strikes + penny-wide spreads + $20-100 credit per leg → ~3% commission drag (vs XSP's 13%). Bare 3-strategy (no IV gate) also passes at ~19% AnnRoC OOS with worst-window Sharpe +0.189 (vs IV<0.25's +0.056) — so bare is safer worst-case, IV<0.25 is higher absolute return. **Recommended paper-trade kickoff**: SPY+PCS+IC+JL+IV<0.25 portfolio at $25K. Caveat: SPY is American-style with physical delivery (early-assignment risk near ex-div, which strategy mgmt rules limit but don't eliminate). 6yr history is short; 2020-COVID heavily weighted in early IS. XSP findings preserved for reference but SPY is now the primary path. |
