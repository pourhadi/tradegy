...
| 14 | Options vol selling | Phases A+B+C+D-1..D-7 complete 2026-05-03; **Phase D-8 (walk-forward + CPCV harness) shipped 2026-05-03 with discriminating findings**. **Bare strategies FAIL the discipline gate** (PCS solo, PCS+IC portfolio, JL+tight all have negative IS Sharpe in every rolling 3y window — each window includes 2022). **IV-gated wrappers SURVIVE**: `PCS + IV<0.30` (counter-canonical — sell vol when IV is LOW, NOT high) PASSES walk-forward at avg IS +0.022 / OOS +0.163; `PCS + IC + IV<0.30` portfolio is the cleanest validated config with avg IS +0.141 / OOS +0.137 (ratio 0.97), all 6 sub-windows positive, ~11.2% AnnRoC during OOS years on $250K. CPCV (45 paths): portfolio + IV<0.30 has 91.1% paths positive (median Sharpe +0.249, vs ungated portfolio +0.036). The canonical practitioner rule "PCS + IV>0.50" FAILS spectacularly. 629 tests passing. Live IBKR contract qualification verified on paper account DU7535411. **Recommended next step**: paper-trade `PCS + IC + IV<0.30` portfolio on $250K, NOT bare PCS or bare portfolio. Caveat: 6yr history is short and 2020-COVID is heavily weighted in early IS; longer history would strengthen the result. |
