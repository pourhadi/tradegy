...
| Phase B-3 — portfolio Greeks + risk caps | ⏳ next — REAL-DATA FINDING (B-2 smoke test): IronCondor45dteD16 entered a single position with $48K capital at risk against the $25K target capital. The runner needs a capital-cap check (refuse to open when total_capital_at_risk would exceed N% of declared capital). Also: portfolio Greeks aggregation across positions, concentration limits per expiration cycle (max 25% per cycle), tail-event halt (suspend new entries when realized vol > 95th pct of trailing year). | `src/tradegy/options/` |
