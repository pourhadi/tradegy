...
| 14 | Options vol selling | Phases A+B+C+D-1..D-7 complete 2026-05-03; **Phase D-8 (walk-forward + CPCV harness) shipped 2026-05-03 with discriminating findings**. **Bare strategies FAIL the discipline gate** (PCS solo, PCS+IC portfolio, JL+tight all have negative IS Sharpe in every rolling 3y window — each window includes 2022). **IV-gated wrappers SURVIVE**: `PCS + IV<0.30` (counter-canonical — sell vol when IV is LOW, NOT high) PASSES walk-forward at avg IS +0.022 / OOS +0.163; `PCS + IC + IV<0.30` portfolio is the cleanest validated config with avg IS +0.141 / OOS +0.137 (ratio 0.97), all 6 sub-windows positive, ~11.2% AnnRoC during OOS years on $250K. CPCV (45 paths): portfolio + IV<0.30 has 91.1% paths positive (median Sharpe +0.249, vs ungated portfolio +0.036). The canonical practitioner rule "PCS + IV>0.50" FAILS spectacularly. 629 tests passing. Live IBKR contract qualification verified on paper account DU7535411. **Threshold robustness sweep**: gate passes monotonically across IV<0.20, <0.25, <0.30, <0.35; fails at <0.40 and at the canonical >0.50. NOT a knife-edge. **Composition sweep at IV<0.25**: 3-strategy PCS+IC+JL portfolio is best — avg OOS Sharpe +0.275, worst OOS Sharpe +0.135 (all 3 OOS windows positive), +$130K OOS over 3y (~17.3% AnnRoC on $250K). **$250K-CAPITAL FINDING — NOT applicable to operator who's deploying $25K**: `PCS + IC + JL + IV<0.25` portfolio on $250K SPX → ~17.3% AnnRoC OOS, but every number above this caveat assumes $250K. **At the operator's actual $25K capital**: SPX is essentially untradeable (RiskManager position-cap rejects most entries — window 2 OOS produced ZERO trades); only XSP+IV<0.25 portfolio survives walk-forward at $25K but at much weaker ~4.5% AnnRoC OOS (+$3.3K over 3 OOS years), with one negative OOS window. The ~12pp gap between SPX@$250K and XSP@$25K matches the cost-economics differential (commissions + spreads ~13% of credit on XSP vs ~0.13% on SPX). **Recommended next investigation**: SPY ETF options (different vendor — would need data acquisition; SPY $100 multiplier + ~1/10-SPX strikes + penny-wide spreads might split the cost-economics difference). Caveat: 6yr history is short and 2020-COVID is heavily weighted in early IS; longer history would strengthen the result. |
